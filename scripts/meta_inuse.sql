...
CREATE TABLE `ddl_ops_log_template_table` (
  ...
  `optype` enum('create','drop','rename','alter','replace') NOT NULL,
...
CREATE TABLE `shards` (
  ...
  `name` varchar(64) NOT NULL,
  `when_created` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `master_node_id` int(10) unsigned NOT NULL,
...
--
-- Dumping routines for database 'pxm'
--
...
BEGIN
    ...
    if @obj_type != 'db' then
        SET @sql1 = CONCAT('select exists(select id from ', tblname, ' where (db_name=? or (objname=? and objtype=\'db\')) and initiator != ? and id > ? lock in share mode) into @conflicts');
    else
        SET @sql1 = CONCAT('select exists(select id from ', tblname, ' where initiator != ? and id > ? lock in share mode) into @conflicts');
    end if;
    ...
    if @obj_type != 'db' then
        ...
        EXECUTE stmt1 USING @initiator_id, @cur_opid;
    end if;
