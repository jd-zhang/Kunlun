...
def install_pg(config_template_file, install_path, compcfg):
    ...
    datadir = compcfg['datadir']
    if datadir[0] != '/':
        raise Error("Invalid data dir: data dir must use full path.")
    if os.path.exists(datadir) and len(os.listdir(datadir)) != 0:
        raise Error("Invalid data dir: data dir " + datadir + " already exists and is not empty!")
    ...
    sql = "set skip_tidsync = true; CREATE USER " + compcfg['user'] + " PASSWORD '" + compcfg['password'] + '\' superuser;'
...
if __name__ == "__main__":
    try:
        ...
        for compcfg in jscfg:
            if install_ids[0] == -1 or install_ids[idx] == compcfg['id']:
                install_pg(config_template_file, install_path, compcfg)
                print "Installation completed for instance in " + compcfg['datadir']
                idx = idx + 1
            if idx == len(install_ids) and install_ids[0] != -1 :
                break
